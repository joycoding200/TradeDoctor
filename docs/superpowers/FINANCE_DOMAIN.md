...
| BREAKOUT | 入场价 > 过去20日最高价 + 成交量>20日均量×1.5 | 向上突破 | — |
| BREAKDOWN | 离场价 < 过去20日最低价 | 向下破位 | — |
| SIDEWAYS | 有均线数据但不满足牛/熊/突破条件 | 震荡市 | — |
...
### PnL 量级分桶（`classify_pnl_level`）

用于 `pnl_distribution` 统计饼图，按盈亏百分比将每笔交易归入一个量级桶。
此分类纯粹基于盈亏数字，与上节 outcome 维度的离场行为标签无关。

| 标签 | 含义 | 阈值 |
|------|------|------|
| 大盈 | 单笔盈利超过 20% | pnl_pct > 20% |
| 正常盈利 | 单笔盈利 5%-20% | pnl_pct >= 5% |
| 小盈 | 单笔盈利 0%-5% | pnl_pct > 0 |
| 小亏 | 单笔亏损 0-8% | -8% <= pnl_pct < 0 |
| 大亏 | 单笔亏损超过 8% | pnl_pct < -8% |

> **注意**：此处的"大亏"（pnl < -8%，PnL 量级）与 outcome 维度的
> LARGE_LOSS_EXIT（亏损 > 20% 离场，行为标签）是完全不同的概念。
> 前者回答"亏了多少"，后者回答"怎么离场的"。
...
| OVERTRADING（高频交易日） | 0.5 | 单日开仓数 > 均值+2σ |
