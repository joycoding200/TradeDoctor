...
3. **分组重建 `build_grouped` 的 `pnl_pct = pnl/invested`，其中 `invested = total_buy_cost + total_buy_comm`**（`backend/app/engine/position.py:285,298`），而 FIFO 重建 `pnl_pct = pnl/(avg_entry*qty + total_buy_comm)`（`backend/app/engine/position.py:138`）。两者分母语义一致（成本+买入费），✅ 但卖出佣金未进入分母——对 Expectancy(R) 这种用 pct 的指标，佣金口径在分母侧不完整。影响小（佣金占比低），但作为研究员会标记。
